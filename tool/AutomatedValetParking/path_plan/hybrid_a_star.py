...
class hybrid_a_star:
    ...
    def calc_node_cost(self, node: Node, father_theta, father_gear) -> np.float64:
        ...
        if node.parent_index==0:
            cost_heading = min(abs(self.pi_2_pi(node.theta - father_theta)),abs(self.pi_2_pi(node.theta - father_theta+math.pi)))
        else:
            cost_heading = abs(self.pi_2_pi(node.theta - father_theta))
            if gear != father_gear:
